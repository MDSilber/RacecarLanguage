...
def analyze(ast):
    ...
    if isinstance(analyzer, basestring):
        ...
        if (ast.type == "WORD"):
            ...
            id = ast.value
            idEntry = table.getEntry(SymbolTableEntry(
                id, None, list(scopeList), function, None))
            if idEntry is None:
                # ID does not exist or exists but the scoping is wrong
                # this is to be returned to binaryOperatorAnalyzer
                return "ERROR"
            if function is None and not idEntry.initialized:
                return "ERROR"
            else:
                return idEntry.type
...
def statementsAnalyzer(ast):
    if firstPass:
        if (ast.children[0].value == "define_command" or
                ast.children[0].value == "statements"):
            analyze(ast.children[0])
        if (ast.children[1].value == "define_command" or
                ast.children[1].value == "statements"):
...
def driveCommandAnalyzer(ast):
    ...
    if result != "number":
        errorList.append("Error in drive command: \
                need to use valid variable or number")
...
def comparisonAnalyzer(ast):
    ...
    if result == "number":
        ...
        if ast.children[1].type == "IS" or ast.children[1].type == "IS NOT":
            ...
            errorList.append("Error in comparison: \
                     words must be compared using 'is' or 'is not'")
    else:
        errorList.append("Error in comparison: \
                use only words or only numbers; cannot mix both")
...
def repeatTimesAnalyzer(ast):
    ...
    if analyze(ast.children[1]) != "number":
        errorList.append("Error in repeat loop: \
                need to use valid variable or number")
    # for "statement_block"
    analyze(ast.children[4])
...
def declarationCommandAnalyzer(ast):
    ...
    table.addEntry(SymbolTableEntry(
...
def assignmentCommandAnalyzer(ast):
    ...
    if idEntry is None:
        idNoneBool = True
        # ID does not exist or exists but the scoping is wrong
        errorList.append("Error1 in assignment: \
                variable does not exist or cannot be used here")
    else:
    ...
    if child3Evaluation == "ERROR":
        ...
        errorList.append("Error2 in assignment: \
                use only words or only numbers; cannot mix both")
    else:
        if (not idNoneBool) and idEntry.type != child3Evaluation:
            # type check failed
            errorList.append("Error3 in assignment: \
                    variable and value must have the same type")


def printAnalyzer(ast):
    # for the word or identifier
    if analyze(ast.children[1]) == "ERROR":
        errorList.append("Error in an expression: \
                use only words or only numbers; cannot mix both")
...
def defineCommandAnalyzer(ast):
    ...
    if scopeList[-1] != 0:
        errorList.append("Error in function creation: \
                functions cannot be created in other \
