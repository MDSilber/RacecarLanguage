...
themselves in their children's education; and \emph{engaging}
...
In order to capture and maintain children's interest
...
shows the car as it executes the program's instructions
...
that captures children's interest. Racecar is designed
...
children's parents could learn how to write, or at
...
the accompanying lessons and debug students' programs
...
new movement (including ``drive straight''
and ``turn'') or programming concept
...
readability. If you thought $\mathtt{OCCURS\ 12\ TIMES}$
means ``loop/repeat 12 times''
in COBOL, you'd be wrong---it's a
declaration for a 12-element array! In Python (a vast improvement
from COBOL), many keywords and functions such as $\mathtt{def}$,
$\mathtt{len}$, and $\mathtt{str}$
...
and there are two primitive types: $\mathtt{number}$
and $\mathtt{word}$. No $\mathtt{ints}$,
$\mathtt{floats}$ or $\mathtt{doubles}$
are around to complicate things, and although $\mathtt{string}$
might be a more general term, $\mathtt{word}$
emphasizes the important difference between the data types more clearly
than $\mathtt{string}$ does. All of Racecar's
...
show students that a precise definition of the car's
...
from the graphical application captures children's
...
Racecar's language and platform combines these approaches,
...
the user. For example, Armor Games' Light Bot (http://cache.armorgames.com/files/games/light-bot-2205.swf)
...
and graphical output. However, LOGO's platform is
not as engaging as children have come to expect from modern software.
Furthermore, LOGO's language itself lacks human-readability
...
since, for example, a proud parent can send his child's
...
and a few menus and buttons. The ``Run Program'' button is what you click when
you want to run the program sitting in the program window. The ``Stop Program''
...
words, and actions (which are called ``variable names''
and ``function names'') cannot
...
``You can write in Racecar'' would
...
you encounter are called $\mathtt{bugs}$, and the art of finding and fixing bugs is called $\mathtt{debugging}$.
...
with a frowny face, $\mathtt{:-(}$, and then continue until the first hyphenated
smiley face, $\mathtt{:-)}$, possibly extending over multiple lines. They are
...
begin with a smiley face, $\mathtt{:)}$, and continue until the end of the line.
...
line as a piece of code---it extends from the smiley face ( $\mathtt{:)}$ ) to
the end of the line.
...
The rules for multiple commands are very simple---you may
already be able to guess them---each command goes on its
...
many languages, comment symbols are boring, like $\mathtt{\#}$ comment, $\mathtt{/{*}}$
...
or if the word "hello" is the same as
the word "good bye", you could say if
2 > 1 or "hello" is "good
bye" \{...\}. Since 2 is in fact greater than 1, the
...
double quote, ", followed by 0 or more characters (i.e.
letters, numbers, or symbols that are not double quotes), followed
by a closing double quote. For example, "Sam said, 'Hello,
world!'" is a valid word, but "Sam said,
"Hello, world!"" is not
because it contains double quotes. Confusingly, it is possible to
have a word that is just a single number: "5"
...
set myNumber to myNumber {*} 2 will store in the myNumber "box"
...
This program also uses the "concatenation"
or "joining" operator, ++. Putting
...
repeat forever! This is called an "infinite loop,"
...
As long as the car's location is not equal to the
home location, location will still be set to "out",
...
can teach your students the name "nickname"
...
used a repeat-times loop instead to "drive 10 steps
and then shift left," 4 times. You can ask your
...
often won't be for others, and it is important to
...
parameter's name and type. This is equivalent to declaring a "normal"
variable using the statement variableName is a "type".
...
\begin{lstlisting}
define shiftLeftThenDriveStraight using numStepsLeft (number) and numStepsStraight (number)
{
    turn left
    turn left
...
are not contained in any function\textemdash{}they are "global."
...
the entire source file---into other functions and outside
...
double-quote. The empty word is notated as "".
...
operator means that the following is still valid: "hello"
++ 5 ++ 2 ++ 3, and the result of the expression is the word "hello523".
To "cast" a number to a word,
simply concatenate it with the empty word, as in ""
++ 5, which evaluates to the word "5".
There is no way to cast a word to a number. Boolean values are not
allowed to be stored in variables, and hence the word "boolean"
...
"console" in the Racecar application.
...
The following listing shows the Racecar grammar, as generated by the PLY parser. The grammar productions are formatted as follows: nonterminals are
in lowercase; terminals are in UPPERCASE; literals are (,),\{, and \}. The empty string
is represented by the production `empty', which produces the PLY <empty> statement. The start symbol for all Racecar programs
is `statements'. Multi-line comments are treated as whitespace by the lexer and
hence are not covered by the grammar.

\lstinputlisting{../Racecar/final_grammar.txt}
...
Our "buzz words" for our language
...
different 'types', without an understanding
...
function name and the variables, all separated by a space (ex. "makeTurn
right 10"). Like Python the language newline-terminated
statements. Again, the goal of these syntax decisions was to attempt
to assimilate the language as much to the children's
...
This was quite easy to do since the language's built-in
...
which are simply 'word' and 'number'.
The language uses the words 'set' and
'to' for assignment (ex. "set
myVar to 10"). There are no foreign words in our
...
to racecar's grammar specification. Any racecar code
...
\begin{itemize}
\item Lexer (Sam Kohn)
\item Parser-AST (Sam Kohn, Jeremy Spencer)
\item Semantic Analyzer (Alex Fields, Colfax Selby)
\item Translator (Sam Kohn, Jeremy Spencer)
\item IDE (Mason Silber, Sam Kohn)
\end{itemize}
...
of things at once, we wanted to make sure that we didn't
step on anyone's toes when making changes, so having
...
language and the targeted demographic. From the IDE's
...
code they would like to run, they press the "Run
Code" button located directly below the coding
text area. Buttons are also available to clear the coding text area
and to reset the car's position. Once "Run
Code" has been pressed, another button labeled
"Stop Program" becomes available,
...
is the area in which the car drives, but more importantly, it's
...
so in the output, if a test did fail, we could tell exactly what wasn't
working.
...
10) """ result = Compiler.getPythonCode(test\_string)
...
a manageable scope, but as the semester progressed, we didn't
...
of our team's organization, but rather a function
...
We would highly recommend future teams use Python. As Sam said, "Python
was the language through which I felt it was least necessary to think
about the rules of the language while coding."
...
\lstinputlisting[caption=\texttt{launchRacecar.py}]{../runRacecarGUI.py}

\subsection{Graphical Interface}

\lstinputlisting[caption=\texttt{racecar.py}]{../Racecar/RacecarGUI/racecar.py}

\subsection{Lexer and Parser}

\lstinputlisting[caption=\texttt{Parser.py}]{../Racecar/Parser.py}

\subsection{Semantic Analyzer and Symbol Table}

\lstinputlisting[caption=\texttt{SemanticAnalyzer.py}]{../Racecar/SemanticAnalyzer.py}

\subsection{Translator}

\lstinputlisting[caption=\texttt{Compiler.py}]{../Racecar/Compiler.py}

\subsection{Tests}

\lstinputlisting[caption=\texttt{Tests.py}]{../Racecar/Tests/test.py}
